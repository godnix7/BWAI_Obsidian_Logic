...
const Appointments = () => {
  ...
  const [appointments, setAppointments] = useState(mockAppointments.filter(a => a.doctor_id === "d1"))
  const [rejectModal, setRejectModal] = useState(null)
  const [completeModal, setCompleteModal] = useState(null)

  useEffect(() => { pageEnter(); setTimeout(() => cardStagger(), 100) }, [])

  const counts = { pending: appointments.filter(a => a.status === "pending").length, confirmed: appointments.filter(a => a.status === "confirmed").length, completed: appointments.filter(a => a.status === "completed").length }
  const filtered = tab === "all" ? appointments : appointments.filter(a => a.status === tab)

  const updateStatus = (id, status, extra = {}) => setAppointments(a => a.map(x => x.id === id ? { ...x, status, ...extra } : x))
    ...
    <DashboardLayout>
            ...
            <div key={apt.id} className="glass-card card" style={{ padding: 20, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                  <h3 style={{ fontWeight: 600 }}>{apt.patient?.full_name}</h3>
              ...
              <div style={{ textAlign: "right", display: "flex", flexDirection: "column", alignItems: "end", gap: 8 }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {apt.status === "pending" && <>
                    <button className="btn-primary" style={{ padding: "6px 14px", fontSize: 12 }} onClick={() => updateStatus(apt.id, "confirmed")}><Check size={14} /> Approve</button>
      ...
      {rejectModal && (
        <Modal title="Reject Appointment" onClose={() => setRejectModal(null)}>
          ...
          <textarea className="input" placeholder="Explain why this appointment is being rejected..." id="reject-reason" />
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 12, marginTop: 16 }}>
            <button className="btn-ghost" onClick={() => setRejectModal(null)}>Cancel</button>
            <button className="btn-danger" onClick={() => { updateStatus(rejectModal.id, "rejected", { rejection_reason: "Schedule conflict" }); setRejectModal(null) }}>Reject</button>
          </div>
        </Modal>
      )}
      ...
      {completeModal && (
        <Modal title="Complete Appointment" onClose={() => setCompleteModal(null)}>
          ...
          <textarea className="input" placeholder="Add notes about the visit..." />
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 12, marginTop: 16 }}>
            <button className="btn-ghost" onClick={() => setCompleteModal(null)}>Cancel</button>
            <button className="btn-primary" onClick={() => { updateStatus(completeModal.id, "completed", { notes: "Patient stable." }); setCompleteModal(null) }}>Mark Complete</button>
          </div>
        </Modal>
      )}
    </DashboardLayout>
